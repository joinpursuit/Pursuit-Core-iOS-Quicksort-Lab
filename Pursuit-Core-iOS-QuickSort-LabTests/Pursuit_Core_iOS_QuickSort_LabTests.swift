//
//  Pursuit_Core_iOS_QuickSort_LabTests.swift
//  Pursuit-Core-iOS-QuickSort-LabTests
//
//  Created by Benjamin Stone on 10/29/19.
//  Copyright © 2019 Benjamin Stone. All rights reserved.
//

import XCTest
@testable import Pursuit_Core_iOS_QuickSort_Lab

class Pursuit_Core_iOS_QuickSort_LabTests: XCTestCase {
    ...
    func testQuickSort() {
        for testCase in qOneAndThreeTestCases {
            let arrayUnderTest = quickSorted(arr: testCase.collection, by: <)
    ...
    func testBubbleSortWithoutMovingNegatives() {
        for testCase in qTwoTestCases {
            let arrayUnderTest = quickSortedWithoutMovingNegatives(arr: testCase.collection, by: <)
            XCTAssertEqual(arrayUnderTest, testCase.sortedCollection, "Was expecting \(testCase.sortedCollection), but got \(arrayUnderTest)")
        }
    }
    ...
    func testCocktailSort() {
        for testCase in qOneAndThreeTestCases {
            var testCaseCollection = testCase.collection
            quickSortInPlace(arr: &testCaseCollection, by: <)
            XCTAssertEqual(testCaseCollection, testCase.sortedCollection, "Was expecting \(testCase.sortedCollection), but got \(testCaseCollection)")
        }
    }
    
    func testBubbleSortLinkedList() {
        for testCase in qFourTestCases {
            let listUnderTest = quickSorted(list: testCase.collection, by: <)
